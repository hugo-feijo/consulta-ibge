(ns consulta-ibge.service
  (:require [io.pedestal.http :as http]
            ...
            [consulta-ibge.logic.json :as l.json]))

(defn get-city-by-name
  [request]
  (if-let [city-name (get-in request [:query-params :nomeCidade])]
    (l.json/response-json-content (l.cities/get-city-by-name city-name))
    (ring-resp/bad-request "Provide query nomeCidade")))

(defn get-all-cities
  [request]
  (l.json/response-json-content (l.cities/get-all-cities)))
